...
>
> 最后同步 HEAD: 本会话 docs 提交（2026-08-27，Windows 机：刷新项目文档——design.md 全量重写为 v3 当前架构（废弃 WebSocket 旧描述，落桥协议 8 符号/能力矩阵/构建链）+ 新增 features.md 面向用户功能介绍；基线 dc2e4a93f5 代码无改动）
...
29. **非 deepseek 端点泄漏 DeepSeek 私有 wire 字段与自定义头 → 400/401**（本会话修复，2026-08-26 Mac，用户真机报 kimi-k3 invalid authentication + gemini-3.6-flash http 400）：三端共用 llm-deepseek OpenAI 兼容适配器，harness-entry 写的是默认 `emitThinking: name === 'deepseek'` 但 `RequestDefaults` 接口**没有**这个字段名（拼写错位：应叫 `thinking`，后来写 `emitWireThinking`/`emitReasoningEffort`/`emitHarnessHeaders` 三个新开关时忘记同步**真正的编译入口 harness-entry 位置**——`tools/harness-transpiler/src/harness-entry.ts` 会**覆盖**子模块里同名文件），导致 defaults 始终 undefined，`resolveThinking` 把 `reasoning_effort: high` 与 `thinking.enabled` 无条件发给所有端点。Gemini OpenAI-compat 对未知顶层字段回 HTTP 400，Moonshot 等中国网关对 `x-deepseek-harness-user-id` 自定义头触发鉴权策略 401。修复：①`RequestDefaults` 扩 3 个布尔开关 + ②`resolveThinking` 按开关隐藏 thinking/reasoning_effort + ③adapter.request() 按开关隐藏自定义头 + ④**真正的编译入口**（tools/harness-transpiler/src/harness-entry.ts）按 name 分发开关（仅 deepseek 全开，其余全关）；修复前必须用诊断脚本抓 fetch 请求（模拟 engine.chat）确认三个泄漏点都消失再分发三端。**陷阱 13 变体再变体**：不仅子代理报告≠落盘，修改文件也必须先搞清楚「实际被编译的是哪一份路径」——build.mjs 会把 transpiler/src/*.ts 复制覆盖到子模块目录再 esbuild，改子模块那份就白改。**纪律**：harness-entry/device-tools/capability-manual 的 TS 源码永远在 tools/harness-transpiler/src/ 改，子模块同名文件是覆盖产物；llm-deepseek serialize.ts/adapter.ts 才在子模块 packages/llm 改。
...
- 2026-08-26 Mac 端 kimi/gemini 401/400 兼容修复批次（bf39b7be2f）：用户真机反馈——「kimi-k3 报 invalid authentication、gemini-3.6-flash 报 deepseek api error（http 400）、报错后①底部水位条（10.5k/65.5k）停留旧值②模型选择页全空（鸿蒙同现，glm-5.3 仍正常发送说明引擎没死）」。①**根因定位**（陷阱 29）：三端共用 OpenAI 兼容适配器，但编译入口在 `tools/harness-transpiler/src/harness-entry.ts`（build.mjs 会覆盖子模块同名文件），之前写入的是不存在于 `RequestDefaults` 接口的 `emitThinking` 字段，导致 `resolveThinking` 无条件给所有端点发 `thinking.enabled` + `reasoning_effort: high`；自定义头 `x-deepseek-harness-user-id` 也被无条件发送。Gemini v1beta/openai 兼容端点对未知顶层字段返回 400；Moonshot 对未知自定义头触发鉴权 401。②**修复**：`RequestDefaults` 扩三开关（`emitWireThinking`、`emitReasoningEffort`、`emitHarnessHeaders`）+ `resolveThinking` 按开关隐藏 wire 字段 + adapter.request() 按开关隐藏头 + buildConnection 分发（仅 deepseek 全开，其余全关）；③**验证**：写 Node 模拟 fetch 诊断脚本（verify-fix.mjs）抓三条请求，moonshot/gemini 三嫌疑项全 absent、deepseek baseline 全 present（✅ 3/3）；QuickJS 门禁 ✅；冒烟 11/11 ✅；iOS build 0 error（仅有旧 onChange deprecation warn）+ XCTest 14/14 ✅；三端 harness.js SHA256 一致。④**伴生现象评估**：glm-5.3 报错后仍正常 → 内核无损；Token 水位条"卡住"= refreshUsageStats 失败或未被触发（旧值未清，属轻微 UI 表象）；模型页为空 = 错误后 listProviders 桥接失败（iOS LocalEngine.swift 已加 NSLog 探针，等真机验证。关联结论：若 glm-5.3 正常 chat，callFunc 通路未死，可能是 jsQueue 有迟到事件导致 evaluateScript 返回 nil；也可能是 api_key 为空被 listUsableProviders 过滤）
- 2026-08-26 Windows 端更新代码批次（本会话）：用户指令「更新代码」→ fetch 发现远端 Mac+Qoder 并行推 10 笔（dafb3ba475→dc2e4a93f5）：内核升级 dsh-v0.1.1-rc.2 / 沙箱定时器+bg_timer / **内核能力补齐阶段 1**（fs 三件套+web_fetch+subagent 进 bundle，harness.js 2284KB）/ reasoningEffort low 档三端 / 模型 ID 预设刷新（deepseek-v4 系等八家）/ stream failed 详情透出（陷阱 28）/ kimi/Gemini 401/400 修复。fast-forward 干净合流，无冲突。验证：三端 harness.js SHA256 一致（11B74F57…，2403739 字节）+ bundle grep（reasoning_effort×21/detail×61 在产物）+ iOS Providers v4 系在 + Android compileDebugKotlin ✅ + 鸿蒙 assembleHap BUILD SUCCESSFUL 35s（含 C++ 重编译+SignHap）。CI dc2e4a93f5：Build HarmonyOS 5m25s ✅ + Build iOS 9m19s ✅（Android 随 b5d489e0be 3m49s ✅）。新待办：reasoningEffort low 档真机抽验、fs/web_fetch/subagent 三用例真机验、模型验证失败详情透出真机验
- 2026-08-27 Windows 端文档刷新批次（本会话）：用户指令「根据最新能力，刷新整个项目设计文档、功能介绍」→ 调研确认旧 design.md（v2，2026-08-17）严重过时（仍描述 WebSocket/JSON-RPC 主链路 + HarmonyOS NAPI stub + Android/iOS 骨架的 Phase 2 愿景，全已废弃）；search 子代理产出当前架构事实清单（带文件路径行号）后**全量重写 doc/design.md 为 v3**：三端内嵌 JS 引擎直跑 harness.js（iOS JSC/Android QuickJS wrapper/鸿蒙自编译 QuickJS NAPI）、桥协议同构（8 上行符号+下行五段 fetch 回调+__harnessCall 唯一驱动入口）、内核 API 面 14 项（harness-entry.ts 1397 行 compose 含阶段 1 新进件 fs/web/subagent）、沙箱桥 __sb* 十符号+prelude OOXML、三段式构建链、功能矩阵与端差异（iOS 设备工具 9/21 为最大缺口、鸿蒙独有消息排队、数学公式三端降级）；**新增 doc/features.md**（面向用户的功能说明书：对话体验/工具链/设备能力/模型供应商/隐私安全/已知限制六节）。纯文档改动无代码变更，无需门禁；doc/ 在 .gitignore 内需 `git add -f`
